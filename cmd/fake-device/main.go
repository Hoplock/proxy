...
func run() error {
	...
	if *debug != "" {
		// The same shape cmd/mock-control's /debug endpoints have, and for the
		// same reason: a real appliance's administrator table is read over its
		// own CLI, and a scenario that had to drive a CLI to check whether the
		// proxy cleaned up would be asserting on the thing it is testing. The
		// accounts live in this process's memory, so there is no file to read
		// instead.
		mux := http.NewServeMux()
		mux.HandleFunc("/debug/accounts", func(w http.ResponseWriter, r *http.Request) {
			names := make([]string, 0)
			for name := range dev.Accounts() {
				names = append(names, name)
			}
			sort.Strings(names)
			w.Header().Set("Content-Type", "application/json")
			_ = json.NewEncoder(w).Encode(map[string]any{"accounts": names})
		})
		srv := &http.Server{Addr: *debug, Handler: mux, ReadHeaderTimeout: 5 * time.Second}
		go func() {
			if err := srv.ListenAndServe(); err != nil && !errors.Is(err, http.ErrServerClosed) {
				logger.Printf("fake-device: debug listener: %v", err)
			}
		}()
