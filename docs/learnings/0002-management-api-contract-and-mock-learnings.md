# 0002 — Control API contract & mock server — Learnings

## Summary
- What shipped: the Control API contract (`api/control.yaml`, OpenAPI 3 +
  `api/README.md`), the typed Go client and contract types in `internal/control`,
  and a fixture-driven mock server in `cmd/mock-control` with debug endpoints
  for tests. No proxy-side consumption yet.
- Key packages/files: `api/control.yaml`, `api/README.md`,
  `internal/control/{contract,client,rest,errors}.go`,
  `cmd/mock-control/{server,fixtures,main}.go`,
  `cmd/mock-control/fixtures.example.yaml`, `Makefile` (`openapi-check`),
...
- Endpoints (7): `POST /v1/auth/cert`, `POST /v1/auth/password`,
  ...
  constants: `control.PathAuthenticateCert`, `PathAuthenticatePassword`,
  `PathPollMFA`, `PathAuthorize`, `PathReportHostKey`, `PathIngestLogBatch`,
  `PathIngestPriorityLog`.
- Key types: `control.Client` (7 methods, all `(ctx, *Req) (*Resp, error)`:
...
- What the NEXT session (0004) must know: use `control.Client`, never HTTP; build
  `internal/identity` from `control.Identity` (the wire DTO) and convert at the
  control boundary; the cert-first-then-password+MFA order and the MFA polling loop
  are already specified below; run the mock with
  `-fixtures cmd/mock-control/fixtures.example.yaml`.

## Details

### Where the truth lives
`api/control.yaml` is the source of truth; `api/README.md` is the readable
companion (endpoint table, fixture format, mock-only endpoints, and the "how to
change the contract" checklist). `internal/control` mirrors it in Go, and
`internal/control/contract_test.go` fails if they drift: it checks the document is
...
### Auth flow the proxy (0004) implements
...
is the **next proxy** and the user's host travels in `Hop.FinalTarget`; the
server returns the hop trail with the calling proxy appended, which is what
...
worked through in `cmd/mock-control/fixtures.example.yaml` (which a test
...
### Test approach
`cmd/mock-control/server_test.go` drives the mock **through the real
`control.RESTClient`**, so each test exercises contract, client, and server
...
batch + priority ingest with de-duplication, proxy-token enforcement, and
fixture validation. `internal/control/rest_test.go` covers the client alone against
...
to Hoplock Control — a blocked command is a local list match. Setup costs
...
server-set TTL on `AuthorizeResponse`, never a proxy-side TTL, and why the
...
- `internal/identity` (0004) should own the proxy's identity model and convert
  to/from `control.Identity`; `control.Identity.Claims` is `map[string]string` on the
  wire, so a richer internal claims type must serialise into that shape or the
  contract changes with it.
- The proxy→server credential is a bearer token stub (`Options.Token`, no
  config field yet). Whichever phase wires the real client into the proxy
