...
| **D18** | Fleet configuration: which settings Control may distribute, how a document arrives, what "running" claims, and that a bad one never takes a proxy out of service | new (0042) | §6.4, §8, `api/`, `config.example.yaml` |
...
- **D18 — Configuration above the bootstrap is the fleet's; the bootstrap is
  the host's (new, phase 0042).** D2 says the proxy originates no *policy*.
  Configuration is not policy, so D2 neither grants nor forbids distributing it,
  and until this entry the register was silent. Hoplock Control holds a
  versioned configuration document per proxy (its own registry, composed from
  zone and proxy versions, with rollback); this decision says what a proxy
  takes from it.

  **The line.** A setting stays **bootstrap** — settable only in the host's
  YAML — if the proxy needs it to **reach Hoplock Control at all** or to be
  recognised by it (`control.base_url`, `control.token`, `proxy.id`); if it
  **names material on the host** (every key, certificate, directory and
  environment-variable name); if it **binds a listener** (`proxy.listen_addr`,
  `chain.accept.listen_addr`); or if it is the proxy's **own judgement of
  whether it can still hear Control** (`control.cache.stale_after` — the one
  thing Control cannot vouch for about itself, the same reasoning as §6.4's
  tighten-only heartbeat advertisement). A proxy that could be told any of
  those remotely is one a bad document can orphan permanently, with no channel
  left to repair it over. Everything else is a *candidate*, and becomes
  **fleet-owned** only by being listed — so a new setting is bootstrap until a
  phase decides otherwise, and a document cannot widen the list by accident.
  Fleet-owned today: `control.cache.max_ttl` and `session.deadline_warning`
  (**live**), and — at startup only — `control.cache.max_entries`, the
  `logging.*` throughput knobs (D8), `chain.max_hops`, `chain.upstream.address`
  (D11's registration target, whose host key stays bootstrap), `dial.*`, and
  `auth.user.mfa.*`. `internal/config/fleet.go` is the list,
  `config.example.yaml` marks each setting where an operator reads about it,
  and a test keeps the two equal.

  **How it arrives.** A `config_changed` event on the revocation stream
  **names** the desired document (version, hash) and never carries it: the
  stream is replayable, and a replayed document is a stale one applied as if
  current. The proxy fetches the **current** document from
  `GET /v1/proxies/{proxy_id}/config` — conditionally, on the hash it holds, so
  unchanged is a `304` — on each notification that names something new, on every
  stream (re)connect and `resync`, and at startup before building anything. A
  replayed notification therefore costs at most a `304`.

  **What "running" claims.** A document is applied **whole or not at all**, and
  a process runs exactly one. It is reported running
  (`POST /v1/proxies/{proxy_id}/config/report`) only when every setting in it is
  in force; one that changes a startup-only setting is held, applies nothing,
  and is reported `pending_restart` with the settings named. Control's drift
  view cannot then say a rollout finished that has not.

  **A bad document never takes a proxy out of service.** One that cannot be
  parsed or applied — including one naming a bootstrap setting — is
  `rejected`; one that cannot be fetched is `fetch_failed` and retried. Either
  way the proxy keeps serving on its running document, logs it, and reports it
  with a `last_error` for Control's fleet view. Configuration is not on the data
  path: nothing here ends a session, refuses a connection, or touches a cached
  decision, and a session in flight stays governed by the snapshot it was
  authorized with (D2). The running document is not persisted; a proxy that
  restarts while Control is unreachable starts on its bootstrap file.

  **What it does not move.** `policy_version` governs `/v1/authorize` alone, and
  a new event type is licensed by "a proxy ignores a type it does not
  recognise" — so the contract goes to 4.2.0 and the vocabulary stays at 4.
...
**What the deny branch does not cover: the flow that led to it** (phase 0034).
The wording is vague; the flow around it is not. A correct password is answered
with an out-of-band challenge and a wrong one is refused outright, so a client
can read the first factor off whether a challenge appeared — and off how long
the attempt took — without any message disclosing it. That is **accepted, not
overlooked.** Whether a challenge is issued is Hoplock Control's decision and
the proxy must never acquire one (D2); the contract says so too, since a `200`
on `POST /v1/auth/password` means the password was accepted. The mitigation — a
decoy challenge for a login that will never be approved — buys a narrowing
rather than a closure (the resolution *timing* still separates a human from a
synthetic draw), and it turns every failed guess from one Control call into
~121 and from a stateless rejection into a connection held for the challenge's
life. What blunts enumeration here is a **rate**, not a signal, and rate
limiting belongs to Control and to the IdP it federates with (§12). Reasoning:
`docs/learnings/0034-mfa-challenge-first-factor-oracle-learnings.md`, which also
names the contract sentence a future reversal must relax first.
...
**When a hop refuses the key (phase 0033).** A far hop that will not accept this
proxy's chain identity — a fingerprint its Hoplock Control does not recognise, a
key rotated here and never registered there — is the trust model above
**working**, and it is reported as its own failure rather than as an unreachable
proxy: its own `hop-auth` stage, an outage saying the proxy was not accepted and
naming neither the hop nor the key (§4.3), and a critical audit record carrying
the next proxy id, the hop direction and the key's SHA256 fingerprint. The
classifier is the one `internal/auth/target` already owns; there is exactly one
copy of x/crypto's wording in the tree, and a host-key failure still wins where
both apply.

It is deliberately **not contained**, and the asymmetry with §5's target-leg
breaker is the reason. That breaker protects against a property of an OpenSSH
*target* — per-source penalties scoring a refused credential against the proxy's
single source address, so one route's stale key blocks the target for everybody.
The far end of a chain leg is another Hoplock proxy on `x/crypto/ssh`, which has
no such defence, so that blast radius does not exist; a `relay` hop opens no new
connection at all, so there would be nothing to withhold on half of D11; and a
refused chain key stays refused until an operator registers it, which makes a
cooldown a *second* outage stacked on the first, between their fix and service
returning. Classification and an immediate record are what shorten that loop.
...
The same stream carries one event that is **not** about revocation:
`config_changed` (D18, phase 0042) says the proxy's desired configuration
document moved. It is a notification only — the document is fetched, never
carried, because this stream is replayable — and it touches no session and no
cached decision. A proxy with no configuration handler ignores it like any type
...
- **A deadline that has already passed ends the session on arrival, and it is
  ordinary rather than exotic** (phase 0041). The two properties above compose
  into it: the instant is absolute, and an authorize decision may be **reused**
  for as long as the server's cache hint allows (D2, §6.4), so a decision
  replayed `ttl_seconds` later replays the instant it was computed with. Where
  that instant came from a just-in-time grant's expiry, it can be behind the
  proxy's clock before the connection it serves is opened — which is the
  property an absolute instant was chosen **for**, since a duration would
  silently re-anchor instead. So it is **not** a contract violation and never a
  denial or an outage: the session ends the way every other expiry does, as
  §4.3's third case, with the same `end_reason` and the same text. The check is
  **synchronous** and runs where the deadline is resolved, in front of capture,
  concurrency, provisioning and the target dial — the timer would reach the same
  ending, but it would race setup for it, and what is torn down would then
  depend on the scheduler rather than on a rule.
...
- **Config**: YAML bootstrap (`internal/config`), documented with an example file.
  Settings above the bootstrap may also be set fleet-wide by a document Hoplock
  Control distributes (**D18**): the fleet-owned ones are listed in
  `internal/config/fleet.go` and marked `[fleet: live]`/`[fleet: restart]` in
  `config.example.yaml`; every unmarked setting is bootstrap-only.
...
| 0038 | A channel request is answered before its channel closes | a real defect surfaced as a test flake in phase 0037's verification: `pump` gates teardown on the output draining, but `all.Wait()` — the only thing that waits for the CLIENT-side request forwarder — runs *after* `near.Close()`, so a request the target has already accepted can have its channel closed before the reply is relayed. A client `exec` on a fast-exiting command then sees the channel end instead of the `true` it was waiting on, which is indistinguishable on the wire from the refusal a denied command gets (§4.3, §6.3). The proxy already holds this ordering for the exit status — captured and replayed after the output drains — and the request reply never got the same treatment. Reproduces at roughly 1 run in 70 on `TestCIMayRunCommandsButNeverGetsATerminal` and `TestAPTYRequestRecordsTheReplayHeader`, both of which send `pty-req` then `exec` on one channel. The fix must stay **bounded by `exitGrace`**: moving `all.Wait()` above the closes deadlocks, because `forwardClientRequests` ranges over a channel that does not close until the channel does |
| 0039 | Observable liveness and durability      | two obligations this contract states but did not make **checkable**, found while `hoplock/control` phase 0002 wrote the black-box conformance suite: heartbeats were required "at a steady interval" with no interval on the wire, so a suite could only grade a number a human typed into an expectation file; and nothing here publishes an event or reads a log record back, so the priority ack's durability and gap recovery's no-skip promise are not gradeable from this contract at all. **Delivered:** `RevocationEvent.heartbeat_interval_seconds` (**contract 4.1.0**; `policy_version` stays **4**, because the number governs `/v1/authorize`, the one response decoded strictly, and this field is on another endpoint), with its three rules stated beside it — absent means the proxy keeps its own timers, the value may only ever **tighten** detection and never extend a timeout (the idiom of `cache.ttl_seconds` and `report_after_seconds`), and the **10s ceiling** stays, at half the proxy's 20s reconnect timeout so two consecutive intervals fit inside it. `cmd/mock-control` advertises the interval derived from the fixture key that drives its own ticker, so it cannot claim one interval and keep another. The second gap is answered by two paragraphs rather than two endpoints: neither reading a log back nor publishing an event is proxy-facing, so `api/README.md` says so and names `GET /debug/logs` and `POST /debug/revoke` as the reference shapes a harness takes as **inputs**, both staying mock-only. Contract change — carried a cross-repo obligation |
...
| **0034** ⊘ | *(never renumbered)* | Does the MFA challenge disclose the first factor? |
...
**29 of 45** are. `0031` is the session-bounds phase today and is what the
