...
- **A renumber is only done with its mapping written down.** Renumbering is
  ...
  **Do not compose those notes by hand.** They stack, there are eleven, and the
  ...
  alone: 29 of the 38 numbers in use are simultaneously a live phase and a
...
- **A withdrawn number is retired for good** — never reused, even though nothing
  ...
  `docs/learnings/0021-…`, `docs/learnings/0030-…`, `docs/learnings/0032-…` and
  `docs/learnings/0034-…`). Those are the gaps the contiguity rule above does not
  ...
  **Withdrawal is a real outcome of a session, not a failure to deliver.** All
  four so far ended in a reasoned decision not to build, written up in
