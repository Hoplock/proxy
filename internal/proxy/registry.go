...
// DefaultKillReason is shown when the management server ordered a kill without
...
const DefaultKillReason = "your session was ended by the management server"

// The Server implements mgmt.SessionRegistry, which is the half of the
...
// this bastion does not have. A kill for an unknown session is normal — the
// server broadcasts to a fleet and does not track which bastion holds which
// session — so treating it as an error would fill the revocation stream's logs
// with noise and tell the operator nothing.

// KillSession implements mgmt.SessionRegistry.
...
// KillSubject implements mgmt.SessionRegistry.
//
// It matches on the identity's subject, never on the login: the login is what
// the user typed, while the subject is the stable id the management server
...
// KillAll implements mgmt.SessionRegistry.
