...
- `docs/learnings/0002-...-learnings.md` — the contract, the `control.Client`
...
## Objective
Let Hoplock Control authorise a proxy to **reuse an authorize decision
...
- **Revocation stream**: `GET /v1/bastions/{bastion_id}/events` — a long-lived
  ...
  proxy, because proxies sit behind firewalls and must not need an inbound
  ...
  - `resync` — "you missed events": the proxy drops its entire cache and
    re-authorizes from scratch.
- **Gap recovery**: the proxy sends the last `event_id` it processed on
...
- `RevocationStream` — subscribes to the events endpoint:
  `Subscribe(ctx context.Context, proxyID string) error`, reconnecting with
...
### 3. Mock server (`cmd/mock-control`)
...
- `POST /debug/revoke` (mock-only, alongside the existing debug endpoints):
  takes an event body and delivers it to subscribed proxies, so tests and the
...
- A poll-based fallback for proxies that cannot hold a long-lived connection.
- Server→proxy push over anything other than this stream (no inbound listener).

## Acceptance criteria
- `api/control.yaml` still validates (`make openapi-check`) and the contract
  test in `internal/control` covers the new path and the new enums.
