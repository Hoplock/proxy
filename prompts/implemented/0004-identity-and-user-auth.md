# 0004 — Identity model & user→proxy authentication

## Read first
- `docs/PROTOCOL.md` — session workflow.
- `docs/PLAN.md` — especially §2 (D4), §4.1 (user→proxy interface), §4.3
  (what the user is told), §7 (redaction).
- `docs/learnings/` — read summaries; open `0002` learnings (control client +
  auth endpoints) and `0001` learnings (config).

## Objective
Define the AD/Okta-ready **identity/claims model** and implement the
**user→proxy** authentication plane as a pluggable interface with two
...
- `internal/auth/user`: the `UserAuthenticator` interface from PLAN §4.1, plus:
  - `cert` authenticator: validates the client-offered public key/cert by calling
    Hoplock Control's **authenticate (cert)** endpoint; returns `Identity`.
...
- **User-facing feedback during auth (PLAN §4.3).** A failed setup must never be
  ...
  - Use `ssh.ServerConfig.BannerCallback` to tell the user what is happening
    while the proxy talks to Hoplock Control.
  ...
  - Apply the disclosure split when auth fails: a deny (`control.IsUnauthorized`)
...
- `UserAuthenticator` interface + both implementations compile and are unit-
  tested against the mock Hoplock Control (`httptest` or `cmd/mock-control`).
