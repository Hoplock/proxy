# 0002 — Control API contract & mock server
...
interface in the system) and ship a **mock/reference Hoplock Control** plus a
...
- **Contract in `api/`** (source of truth): document each endpoint with request/
  response JSON schemas. Use an OpenAPI 3 document (`api/control.yaml`) plus a
  short human-readable `api/README.md`. Cover:
  1. **Authenticate (cert)** — proxy submits offered public key/cert + conn
     metadata; server returns an **identity/claims** object or 401.
  2. **Authenticate (password + MFA)** — proxy submits username + password +
     conn metadata; server performs out-of-band MFA and returns identity or 401.
     Model MFA as the server's concern; the proxy just relays and polls/waits
     per the contract.
  ...
  4. **Report host key** — proxy reports a newly seen target host key (D7).
  ...
  6. **Ingest logs (priority/immediate)** — accepts a single critical record
     ...
     decide and document.
- **Contract types in `internal/control`**: Go structs for every payload, plus a
...
- **REST client implementation** (`internal/control`, JSON over HTTPS, D9):
  context-aware, timeouts, typed errors (distinguish 401 from transport errors),
  and a clear seam for auth of the proxy→server channel (stub/token for now).
- **Mock server** (`cmd/mock-control`): implements the contract from a static
...
- Proxy-side consumption of these calls (auth/proxy phases consume them).

## Acceptance criteria
- `api/control.yaml` validates as OpenAPI 3.
- `internal/control.Client` interface + REST implementation compile and are
  unit-tested against an `httptest` server.
- `cmd/mock-control` serves all endpoints; a test drives it end-to-end for:
...
`control.Client` interface signature, and how to configure the mock's fixtures —
