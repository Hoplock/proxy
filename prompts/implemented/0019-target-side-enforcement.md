# 0019 — Target-side enforcement: confining the ephemeral account

> Renumbered from 0014 by the privileged-access revision (PLAN §10) and widened
> there to cover device accounts as well as POSIX ones. Queued after 0018 for
> the reason 0009 came after 0006: the vocabulary lands first, then the thing
> that enforces it.

## Read first
- `docs/PROTOCOL.md` — session workflow.
- `docs/PLAN.md` — especially §2 (**D12**, as amended by 0018; **D13** for what
  ...
  enforcement-point table 0018 recorded), §4.3 (what the user is told).
- `docs/learnings/` — read summaries; open **`0018`** (the rung vocabulary,
  ...
  renders onto the target), **`0014`** (what a FortiOS access profile can
  actually constrain — the device rung is rendered from that, not from memory),
  **`0015`** (which corrected several of 0014's FortiOS facts and removed the
  fixed access-profile default this phase was originally told to replace: there
  is no default, so what you replace is a required proxy-wide setting), and
  **`0016`** (what a device account's *scope* is on a unit with virtual domains
  — rendering a rung onto "a device account" needs to know which one).
...
see that traffic at all (0018 records why). Egress confinement is the half that
...
The provisioner gains, per rung named by 0018's contract **on each axis**, the
...
- **Shell and `PATH` confinement** for the rung whose guarantee is a guardrail
  ...
  and in the learnings, in the words 0018 chose. Do not let it drift into being
  described as a boundary.
- **Filesystem confinement** where 0018's table says a rung requires it: a
  per-session home mounted `noexec,nosuid,nodev`, or a mount namespace. Only
  build this if 0018 named a rung that needs it.
- **systemd confinement**, if 0018 ranked it where it looks like it ranks: a
  ...
  capability to probe and report (0018), not as an assumption: the directives
  landed across many releases and a silently-ignored one is a rung that claims a
  guarantee it is not delivering.
- **Egress confinement** for the reach axis, by whichever mechanism 0018 chose:
...
- **A rung the proxy cannot provide on this target is an outage-class denial**
  ...
  have. The proxy advertises what it can provide (0018); this is what happens
...
- **Command authorization** is the platform's: an access profile, a privilege
  level, a role, a login class. 0018's vocabulary names rungs by what they
...
- **Pre-provisioned rungs stay pre-provisioned.** A profile that the customer
  defined on the device is attested, not applied, and 0018's contract already
...
### Audit
The rung actually in force is on the session's audit record (0018 defined the
...
- **Applying any rung on a `brokered-key` route.** The target is unmodifiable by
  definition (D6a); 0018's contract makes an *applied* rung on such a route an
  ...
  **attested** rung — the appliance enforcing its own roles or privilege levels,
  which 0018 defines — is not this phase's to apply either: nothing is
  ...
  covers exactly that.
- Shipping SELinux/AppArmor policy modules for customer fleets. If 0018 named a
...
- **UID allocation.** Phase 0024 owns it. The two phases are halves of one fix —
  0024 stops a fresh account inheriting a torn-down one's uid, and the
  ...
  confined account can still leave behind, because 0024's author needs it.
...
image 0012 unified), for each rung 0018 named:
...
`docs/learnings/0019-target-side-enforcement-learnings.md`. Summary block MUST
