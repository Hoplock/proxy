# 0008 — Channel allow-list & inspection pipeline

## Read first
- `docs/PROTOCOL.md` — session workflow.
- `docs/PLAN.md` — especially §2 (D5), §6.2 (channel enforcement + inspection
  pipeline; low-latency requirement).
- `docs/learnings/` — read summaries; open `0005` (how channels/requests are
  pumped today).

## Objective
Turn the coarse channel handling from 0005 into a proper **inspection pipeline**:
...
the framework with **no heavy inspectors yet** (command filtering is 0009).
...
## Out of scope
- Actual command filtering logic and policy actions (0009). Logging pipeline (0010).

## Acceptance criteria
- All 0005/0006/0007 integration tests still pass unchanged in behavior.
...
`docs/learnings/0008-channel-inspection-pipeline-learnings.md`. Summary block MUST
give the `Inspector` interface signature, how inspectors register per channel
type, and exactly how 0009's command filter should attach to `exec` and `shell`.
