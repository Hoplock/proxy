# 0009 — Logging & telemetry pipeline

## Read first
- `docs/PROTOCOL.md` — session workflow.
- `docs/PLAN.md` — especially §2 (D8), §7 (full logging requirements),
  §4 (redaction).
- `docs/learnings/` — read summaries; open `0008` (audit-event shape + priority
  flag), `0002` (log ingest endpoints), `0004` (basic session logging today).
...
- `internal/logging`:
  ...
  - **Priority/immediate path**: blocked commands and other critical events
    (0008's flagged events) are delivered immediately — either flush the in-flight
...
- Wire capture points in `internal/proxy` / `internal/channel` / `internal/filter`
  to feed this pipeline; remove the interim basic logging from 0004/0008.
...
`docs/learnings/0009-logging-telemetry-pipeline-learnings.md`. Summary block MUST
