# 0010 — Logging & telemetry pipeline

## Read first
- `docs/PROTOCOL.md` — session workflow.
- `docs/PLAN.md` — especially §2 (D8), §7 (full logging requirements),
  §4 (redaction).
- `docs/learnings/` — read summaries; open `0009` (audit-event shape + priority
  flag), `0002` (log ingest endpoints), `0005` (basic session logging today).
...
- `internal/logging`:
  - **Session capture** sufficient to reconstruct a session (PLAN §7): metadata
    (session id, identity + source, target, route/hop path, auth method,
    timestamps, channel types, policy decisions) plus replay-friendly stream
  ...
  - **Priority/immediate path**: blocked commands and other critical events
    (0009's flagged events) are delivered immediately — either flush the in-flight
...
- Wire capture points in `internal/proxy` / `internal/channel` / `internal/filter`
  to feed this pipeline; remove the interim basic logging from 0005/0009.
...
`docs/learnings/0010-logging-telemetry-pipeline-learnings.md`. Summary block MUST
