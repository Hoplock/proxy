# 0017 — Scale harness: replace the arithmetic with measurement

> New phase (privileged-access revision, PLAN §10). It runs **before** 0018
> deliberately: 0018 changes the connection model to solve a load problem, and
...
and so 0018 is justified by evidence or dropped.
...
- **Is the health checking even SSH?** Large network estates are commonly polled
  ...
  the real figure is two or three orders of magnitude lower and 0018 is
...
## Out of scope
- Any change to the connection model — that is 0018, and it is *this* phase's
  output that decides whether 0018 happens.
