...
- `api/README.md` — "Changing the contract" is the procedure this prompt
  follows; `api/control.yaml` is the source of truth.
- `docs/learnings/` — read summaries; open `0002` (contract + client + mock) and
  `0005` (how `Route` is consumed today).

## Objective
Revise the Control API contract so it can **express** the policy this product
...
**This phase changes `api/`, `internal/control`, and `cmd/mock-control` only.**
...
a future Hoplock Control that mints per-session target credentials is another
method, not another breaking change.

Document that a method the proxy cannot perform is an **outage-class denial**
(PLAN §4.3), never a fallback to a different method.

### 5. Hop connection direction (D11, consumed by 0008)
Add a direction to `HopMetadata`: `dial` (this proxy connects to the next) or
`relay` (the next proxy has registered an outbound connection with this one;
open a channel over it). Include whatever the upstream needs to select a
registration (the downstream proxy's id). Document that a `relay` hop with no
...
### 7. Versioning
Decide and document how a proxy and a server that disagree about this
...
absent-value default, so a v2 proxy works against a v1 server. Whatever you
...
### Code changes
- `api/control.yaml` first, then `api/README.md`.
- `internal/control`: types for every new payload, with the same care as 0002
  (named constants for enums, contract test asserting they match the document).
- `internal/control/cache.go`: the deep copy that protects cached decisions from
...
- `cmd/mock-control`: fixture keys for all of the above, with defaults that
...
- Proxy↔proxy relay registration itself (0008 builds it; this phase only
...
- The `internal/control` contract test asserts every new enum value and path
...
- `cmd/mock-control` serves fixtures exercising each new field, and
