...
>
> Pairs with **0019**, which owns the other half (confinement). Neither alone is
> the fix, and this one does not depend on 0019 landing first.
...
- `docs/learnings/` summaries: **0007** (the provisioning scripts, the reaper,
  ...
  a scenario), **0019**'s prompt (the confinement rungs — read its scope so the
...
- **Confining where the account can write.** That is 0019's filesystem rung, and
  ...
  remains exposed until 0019 lands.
...
0019's confinement lands, because that sentence is the honest scope of this fix.
