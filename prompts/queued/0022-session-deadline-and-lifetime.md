# 0022 — The session deadline: enforce it, and tell the user before it bites

> New prompt. Phase **0015** adds the deadline *field* and explicitly defers the
...
>
> **It cannot start before 0015 has merged** — the contract field it enforces
...
- `prompts/implemented/0015-*` — the deadline field it defined, its shape
...
least acceptable exactly when the stream is down. That is the hole 0015's field
...
### 2. What the user is told — 0015 left this open, and it is the point
...
- so work that must outlive a human's session is **not a human's session**. It
  is a machine identity (UC2, D17, 0018) or a job handed to something on the
...
- `api/control.yaml` — 0015 owns the field. Do not amend the contract; if it
...
- Machine-identity connection lifetimes — 0018.
...
`docs/learnings/0022-session-deadline-and-lifetime-learnings.md`. The summary
